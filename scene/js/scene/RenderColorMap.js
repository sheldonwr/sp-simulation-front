export default function ({ lut, controls }) {
  ...
  const textCountBreak = Math.floor(count * 5);
  ...
  var colorText = d3.select("#color-text").append("svg").attr('height', colorTextHeight).attr('width', colorTextWidth + 100);
  ...
  for (let index = 0, colorTextCounter = 0; index < count; index += step, colorTextCounter++) {
    ...
    if (colorTextCounter === textCountBreak || colorTextCounter === 0) {
      colorTextCounter = 0;
      colorText.append('text').attr('x', (index + 1) * barWidth).attr('y', fontSize).text(colorValue).style('font-size', fontSize).style('fill', color);
    }
  }
}